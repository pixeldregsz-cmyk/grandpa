...
def add_message(cid, role, content, username=None):
    ...
    if username:
        conn.execute(
            "INSERT INTO user_profiles (chat_id,username,first_seen,last_seen) VALUES(?,?,?,?) ON CONFLICT(chat_id) DO UPDATE SET last_seen=?,username=?",
            (cid, username, now, now, now, username),
        )
    conn.commit()
    conn.close()
...
def get_user_notes(cid):
    conn = sqlite3.connect(str(DB_PATH))
    row = conn.execute("SELECT notes FROM user_profiles WHERE chat_id=?", (cid,)).fetchone()
    conn.close()
...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "save_file", "description": "Save file.", "parameters": {"type": "object", "properties": {"filename": {"type": "string"}, "content": {"type": "string"}}, "required": ["filename", "content"]}}},
    ...
    {"type": "function", "function": {"name": "list_files", "description": "List files.", "parameters": {"type": "object", "properties": {"path": {"type": "string", "default": "."}}}}},
    ...
    {"type": "function", "function": {"name": "download_file", "description": "Download file.", "parameters": {"type": "object", "properties": {"url": {"type": "string"}, "filename": {"type": "string"}}, "required": ["url", "filename"]}}},
]
...
async def start(u, c):
    ...
    await _safe_reply(u, "*Bot online.*\n\nI run commands, search web, execute code, manage files.\n\n/start /reset /remember")


async def reset(u, c):
    conn = sqlite3.connect(str(DB_PATH))
    conn.execute("DELETE FROM conversations WHERE chat_id=?", (u.effective_chat.id,))
    conn.commit()
    conn.close()
...
async def remember(u, c):
    note = u.message.text.replace("/remember", "", 1).strip()
    if not note:
        await u.message.reply_text("Usage: /remember something")
        return
    save_user_note(u.effective_chat.id, note)
    await u.message.reply_text("Got it!")


async def chat(u, c):
    if not OPENAI_API_KEY:
        await u.message.reply_text("No API key.")
        return
    cid = u.effective_chat.id
    un = u.effective_user.username or u.effective_user.first_name
    ...
    try:
        ...
        add_message(cid, "assistant", reply or "", un)
        await _safe_reply(u, reply)
        for f in files:
            try:
                with open(f["path"], "rb") as fh:
                    await u.message.reply_document(document=fh, caption=f.get("caption") or None)
            except Exception as e:
                await u.message.reply_text(f"Couldn't send file: {type(e).__name__}: {e}")
    except Exception as e:
